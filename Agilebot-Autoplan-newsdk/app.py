...
def _is_robot_idle(state) -> bool:
    """优先按2.0枚举判断空闲状态。"""
    try:
        return state == RobotStatusEnum.ROBOT_IDLE
    except Exception:
        return False


def _robot_status_text(state) -> str:
    """返回可读状态文本用于前端展示与报错。"""
    if hasattr(state, "msg"):
        return str(state.msg)
    return str(state)
...
async def connect_robot(request: Request):
    ...
    try:
        ...
        # 获取控制柜版本（get_version 已废弃，切换到 2.0 推荐接口）
        version_info, ret = robot_arm.get_controller_version()
        if ret != StatusCodeEnum.OK:
            # 获取版本失败时，清理连接状态
            robot_arm.disconnect()
            robot_arm = None
...
async def write_p_data(request: Request):
    ...
    try:
        ...
        status_text = _robot_status_text(state)
        print(f"写入P点前检查 - 机器人运行状态：{status_text}")
        print(f"状态类型：{type(state)}, 状态值（repr）：{repr(state)}")

        # 按2.0状态枚举判断是否空闲
        if not _is_robot_idle(state):
            return JSONResponse({'error': f'机器人当前不处于空闲状态（实际状态：{status_text}），无法写入P点'}, 400)
...
async def check_robot_status(request: Request):
    ...
    try:
        ...
        # 打印机器人状态
        status_text = _robot_status_text(state)
        print(f"机器人运行状态：{status_text}")

        # 按2.0状态枚举判断是否为空闲状态
        is_idle = _is_robot_idle(state)
...
async def auto_write_recipe(request: Request):
    ...
    try:
        ...
        status_text = _robot_status_text(state)
        if not _is_robot_idle(state):
            return JSONResponse({'error': f'机器人当前状态：{status_text}，无法自动写入'}, 400)
...
async def set_do_signal(request: Request):
    ...
    try:
        # 2.0接口推荐使用SignalValue枚举
        signal_value = SignalValue.ON if int(do_value) else SignalValue.OFF
        ret = robot_arm.signals.write(SignalType.DO, int(do_number), signal_value)
        if ret != StatusCodeEnum.OK:
