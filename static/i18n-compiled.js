...
var translations = {
  'zh': {
    ...
    'reference_point': '参考点',
    'reference_point_pr1': '参考点1 (第一行第一列)',
    'reference_point_pr2': '参考点2 (最后一行第一列)',
    'reference_point_pr3': '参考点3 (第一行最后一列)',
  ...
  'en': {
    ...
    'reference_point_pr1': 'Reference Point 1 (First Row, First Column)',
    'reference_point_pr2': 'Reference Point 2 (Last Row, First Column)',
    'reference_point_pr3': 'Reference Point 3 (First Row, Last Column)',
  ...
  'vi': {
    ...
    'reference_point_pr1': 'Điểm tham chiếu 1 (Hàng Đầu, Cột Đầu)',
    'reference_point_pr2': 'Điểm tham chiếu 2 (Hàng Cuối, Cột Đầu)',
    'reference_point_pr3': 'Điểm tham chiếu 3 (Hàng Đầu, Cột Cuối)',
  ...
  'ja': {
    ...
    'reference_point': '参照点',
    'reference_point_pr1': '参照点1 (第1行第1列)',
    'reference_point_pr2': '参照点2 (最終行第1列)',
    'reference_point_pr3': '参照点3 (第1行最終列)',
...
translations.ko = {
  ...
  'reference_point': '참조점',
  'reference_point_pr1': '참조점1 (첫 행 첫 열)',
  'reference_point_pr2': '참조점2 (마지막 행 첫 열)',
  'reference_point_pr3': '참조점3 (첫 행 마지막 열)',
...
var SUPPORTED_LANGUAGES = ['zh', 'en', 'vi', 'ja', 'ko'];
var serverMessageKeyMap = {
...
function normalizeLanguageCode(lang) {
  ...
  var code = String(lang).toLowerCase().replace('_', '-');
...
function initLanguageFromApp() {
  return _initLanguageFromApp.apply(this, arguments);
}
function _initLanguageFromApp() {
  _initLanguageFromApp = _asyncToGenerator(/*#__PURE__*/_regeneratorRuntime().mark(function _callee() {
    var appLang, normalized, currentLang;
    return _regeneratorRuntime().wrap(function _callee$(_context) {
      while (1) switch (_context.prev = _context.next) {
        case 0:
          ...
          _context.next = 4;
          return gbtExtension.getLanguage();
        case 4:
          appLang = _context.sent;
          normalized = normalizeLanguageCode(appLang);
        ...
        case 13:
          ...
          console.warn('Failed to sync language from app:', _context.t0);
        case 16:
          currentLang = getCurrentLanguage();
          lastSyncedLanguage = currentLang;
          applyLanguage(currentLang);
          watchAppLanguage();
          return _context.abrupt("return", currentLang);
        case 21:
        case "end":
...
var lastSyncedLanguage = 'zh';
var languageWatchStarted = false;
function syncLanguageFromAppIfChanged() {
  return _syncLanguageFromAppIfChanged.apply(this, arguments);
}
function _syncLanguageFromAppIfChanged() {
  _syncLanguageFromAppIfChanged = _asyncToGenerator(/*#__PURE__*/_regeneratorRuntime().mark(function _callee2() {
    var appLang;
    return _regeneratorRuntime().wrap(function _callee2$(_context2) {
      while (1) switch (_context2.prev = _context2.next) {
        ...
        case 2:
          _context2.prev = 2;
          _context2.t0 = normalizeLanguageCode;
          _context2.next = 6;
          return gbtExtension.getLanguage();
        case 6:
          _context2.t1 = _context2.sent;
          appLang = (0, _context2.t0)(_context2.t1);
        ...
        case 11:
          ...
          console.warn('Failed to sync language from app:', _context2.t2);
        case 14:
        case "end":
...
function resolveApiMessage(data) {
  ...
  if (data.message_code === 'success_import_result') {
    var _ref, _data$imported_count, _data$params, _ref2, _data$skipped_count, _data$params2, _ref3, _data$id_reassigned_c, _data$params3, _ref4, _data$error_count, _data$params4;
    var importedCount = (_ref = (_data$imported_count = data.imported_count) !== null && _data$imported_count !== void 0 ? _data$imported_count : (_data$params = data.params) === null || _data$params === void 0 ? void 0 : _data$params.imported_count) !== null && _ref !== void 0 ? _ref : 0;
    var skippedCount = (_ref2 = (_data$skipped_count = data.skipped_count) !== null && _data$skipped_count !== void 0 ? _data$skipped_count : (_data$params2 = data.params) === null || _data$params2 === void 0 ? void 0 : _data$params2.skipped_count) !== null && _ref2 !== void 0 ? _ref2 : 0;
    var reassignedCount = (_ref3 = (_data$id_reassigned_c = data.id_reassigned_count) !== null && _data$id_reassigned_c !== void 0 ? _data$id_reassigned_c : (_data$params3 = data.params) === null || _data$params3 === void 0 ? void 0 : _data$params3.id_reassigned_count) !== null && _ref3 !== void 0 ? _ref3 : 0;
    var errorCount = (_ref4 = (_data$error_count = data.error_count) !== null && _data$error_count !== void 0 ? _data$error_count : (_data$params4 = data.params) === null || _data$params4 === void 0 ? void 0 : _data$params4.error_count) !== null && _ref4 !== void 0 ? _ref4 : 0;
    var message = t('success_import_base', {
      count: importedCount
    });
    if (skippedCount > 0) {
      message += t('success_import_skipped_suffix', {
        count: skippedCount
      });
    }
    if (reassignedCount > 0) {
      message += t('success_import_reassigned_suffix', {
        count: reassignedCount
      });
    }
    if (errorCount > 0) {
      message += t('success_import_failed_suffix', {
        count: errorCount
      });
    }
    return message;
...
function translateImportResultMessage(message) {
  var match = message.match(/^成功导入 (\d+) 个配方(.*)$/);
  if (!match) {
    return null;
  }
  var result = t('success_import_base', {
    count: match[1]
  });
  var suffix = match[2] || '';
  var skipMatch = suffix.match(/，跳过 (\d+) 个重名配方/);
  if (skipMatch) {
    result += t('success_import_skipped_suffix', {
      count: skipMatch[1]
    });
  }
  var reassignedMatch = suffix.match(/，(\d+) 个配方重新分配了配方号/);
  if (reassignedMatch) {
    result += t('success_import_reassigned_suffix', {
      count: reassignedMatch[1]
    });
  }
  var failedMatch = suffix.match(/，(\d+) 个配方导入失败/);
  if (failedMatch) {
    result += t('success_import_failed_suffix', {
      count: failedMatch[1]
    });
  }
  return result;
...
function translateServerMessage(message) {
  ...
  var importMessage = translateImportResultMessage(message);
  if (importMessage) {
    return importMessage;
  }
  var exactKey = serverMessageKeyMap[message];
  if (exactKey) {
    return t(exactKey);
  }
  var patterns = [[/^连接失败: (.+)$/, 'error_connect_failed', {
    error: 1
  }], [/^获取型号失败: (.+)$/, 'error_connect_failed', {
    error: 1
  }], [/^获取控制柜版本失败: (.+)$/, 'error_connect_failed', {
    error: 1
  }], [/^机器人当前不处于空闲状态（实际状态：(.+)），无法写入P点$/, 'error_robot_not_idle', {
    status: 1
  }], [/^读取P点 (\d+) 数据失败$/, 'error_read_p_point_failed', {
    id: 1
  }], [/^写入P点 (\d+) 数据失败$/, 'error_write_p_point_failed', {
    id: 1
  }], [/^配方保存成功，配方ID已从 (.+) 重新分配为 (.+)$/, 'success_recipe_saved_reassigned', {
    original_id: 1,
    recipe_id: 2
  }], [/^成功导出 (\d+) 个配方到U盘 \((.+)\)$/, 'success_export_recipes_with_time', {
    count: 1,
    time: 2
  }], [/^成功导出 (\d+) 个配方到U盘$/, 'success_export_recipes', {
    count: 1
  }], [/^配方 (.+) 自动写入成功$/, 'success_recipe_auto_write', {
    name: 1
  }]];
  var _loop = function _loop() {
      var _patterns$_i = _slicedToArray(_patterns[_i], 3),
        regex = _patterns$_i[0],
        key = _patterns$_i[1],
        paramMap = _patterns$_i[2];
      var match = message.match(regex);
      if (!match) {
        return 0; // continue
      }
      var params = {};
      Object.entries(paramMap).forEach(function (_ref5) {
    ...
    if (currentLang[key]) {
      ...
      if (element.tagName === 'INPUT' && element.hasAttribute('placeholder')) {
        ...
        if (element.id === 'robot_connect_button') {
          ...
          var buttonKey = element.getAttribute('data-i18n');
